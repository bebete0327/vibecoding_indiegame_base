...
**이식일**: 2026-04-17
**이식 범위**: 10 agents + 9 skills + 14 engine-reference docs + 5 rules + 2 design docs
**업데이트 2026-04-17**: Option A+B 적용 — TOO-CEREMONIOUS 6개 스킬 제거 + 리뷰/QA/퍼포먼스 에이전트 3개 추가
...
| `lead-programmer` | `/code-review` 의 SOLID/아키텍처 리뷰 리드 |
| `qa-tester` | `/code-review` 의 테스트 커버리지/실행가능성 검토 |
| `performance-analyst` | `/perf-profile` 의 프레임/메모리 병목 분석 |

에이전트 호출은 Claude Code 의 Task 도구로: `"ask godot-gdscript-specialist to review this signal architecture"` 같은 형태.

### Skills (`.claude/skills/` — 슬래시 명령) — 9개
...
| `/code-review` | SOLID + 엔진 패턴 준수 코드 리뷰 | 커밋 전 |
...
**솔로 모드 기본값**: `production/review-mode.txt` = `solo` → 디렉터 게이트 대부분 스킵.

### 제거된 스킬 (팀 워크플로 전용, 솔로 과함)
- `/scope-check` — 프로듀서/PM 도구 (스프린트 기반 조직용)
- `/design-review` — 8+ 스페셜리스트 게이트 스폰 (다인팀 가정)
- `/sprint-plan` — 속도 트래킹 / 팀 캐파 가정
- `/bug-report`, `/bug-triage` — QA 팀 워크플로
- `/prototype` — CD-PLAYTEST 게이트 세레모니

필요 시 CCGS 원본에서 `cp` 로 개별 이식 가능. 솔로 바이브코딩엔 위 9개로 충분하다는 판단.
